...
import { truncate, TruncateOptions } from './truncate';
...
const LEVEL_INFO: Record<number, { name: string; color: string }> = {
...
type PinoRecord = {
  ...
  source?: string;
  label?: string;
  labels?: { companyId?: string; userId?: string; [k: string]: unknown };
  companyId?: string;
  userId?: string;
  requestId?: string;
  err?: ErrShape;
  error?: ErrShape | string;
  stack?: string;
  httpRequest?: HttpRequestLabels;
  loggerName?: string;
  [k: string]: unknown;
};
 ...
 */
export class PrettyDestination extends Writable {
  private muteSources: Set<string>;
  private silencePatterns: RegExp[];
  private dedupeWindowMs: number;
  private dedupeMap = new Map<string, DedupeEntry>();
  private useColor: boolean;
  private truncOpts: Partial<TruncateOptions>;
  ...
  constructor(options: PrettyDestinationOptions = {}) {
    ...
    this.silencePatterns = (options.silencePatterns ?? []).map(
      p => new RegExp(p)
    );
  ...
  _write(
    ...
    while ((idx = this.buffer.indexOf('\n')) !== -1) {
      ...
      try {
        this.handleRecord(JSON.parse(line) as PinoRecord);
      } catch {
  ...
  private handleRecord(rec: PinoRecord): void {
    const source = this.pickSource(rec);
    if (source && this.muteSources.has(source)) return;
    ...
    if (this.silencePatterns.some(p => p.test(msg))) return;
    ...
    const routeKey =
      rec.httpRequest && typeof rec.httpRequest === 'object'
        ? `${rec.httpRequest.requestMethod ?? ''}|${
            rec.httpRequest.requestPath ?? rec.httpRequest.requestUrl ?? ''
          }`
        : '';
  ...
  private flushDedupe(key: string): void {
    ...
    if (entry.count > 1) {
      ...
      this.out.write(
        this.c(GRAY, `         ↳ repeated ×${more} more in ${elapsed}s`) + '\n'
      );
    }
  }

  private pickSource(rec: PinoRecord): string | undefined {
   ...
   * scannable.
   */
  private pickRequestTag(rec: PinoRecord): string {
    const h = rec.httpRequest;
    if (!h || typeof h !== 'object') return '';
    const method =
      typeof h.requestMethod === 'string' ? h.requestMethod : undefined;
    const path =
      typeof h.requestPath === 'string'
        ? h.requestPath
    ...
    const shortPath =
      path && path.length > 40 ? path.slice(0, 37) + '…' : path ?? '';
    const text = [method, shortPath].filter(Boolean).join(' ');
    return this.c(MAGENTA, text);
  }

  private pickIdentityChip(rec: PinoRecord): string {
  ...
  private formatBody(rec: PinoRecord, msg: string): string {
    const lines: string[] = [];
    ...
    if (errBlock) {
      ...
      const errMsg =
        errBlock.message && errBlock.message !== msg ? errBlock.message : '';
      const head = errMsg
        ? `  ✖ ${errMsg}${code}`
        : code
        ? `  ✖${code}`
        : '';
      if (head) lines.push(this.c(RED, head));
      if (typeof errBlock.stack === 'string') {
        const cleaned = stripStackFrames(errBlock.stack);
    ...
    for (const [k, v] of Object.entries(rec)) {
      if (PINO_RESERVED.has(k) || HANDLED_ELSEWHERE.has(k)) continue;
      ...
      extras[k] = truncate(v, this.truncOpts);
    }
    ...
    if (rec.labels && typeof rec.labels === 'object') {
      ...
      for (const [k, v] of Object.entries(rec.labels)) {
        if (k === 'companyId' || k === 'userId') continue;
        remainingLabels[k] = v;
      }
      if (Object.keys(remainingLabels).length > 0) {
        extras.labels = truncate(remainingLabels, this.truncOpts);
      }
    }
    ...
    if (Object.keys(extras).length > 0) {
      ...
      if (compact.length <= 120) {
        ...
        for (const pl of pretty) lines.push(this.c(DIM, '  ' + pl));
      }
    }
   ...
   */
  private extractErrShape(rec: PinoRecord): ErrShape | undefined {
